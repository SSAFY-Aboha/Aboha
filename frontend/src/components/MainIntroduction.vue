<script setup></script>

<template>
  <div class="flex items-center w-4/5 h-full">
    <div class="flex items-start justify-around w-full h-[30rem]">
      <!-- 글자 공간 -->
      <div class="flex flex-col justify-around h-full mt-6">
        <div class="flex flex-col gap-8">
          <h1 class="font-bold text-9xl font-Namum">아보하</h1>
          <h2
            class="pr-4 overflow-hidden text-3xl italic border-r-2 border-r-black animate-typing whitespace-nowrap"
          >
            "아주 보통의 하루, 특별한 여정을 시작하다."
          </h2>
        </div>
        <pre class="pr-4 mt-8 text-lg text-wrap line-clamp-3">
아보하는 평범한 일상 속에서 느끼는 작은 모험과 새로운 경험을 통해,
여행을 꼭 먼 곳에서 하는 것이 아닌 일상의 일부로 즐길 수 있도록 도와줍니다.
      </pre
        >
      </div>
      <!-- 이미지 공간 -->
      <div class="bg-green-100 w-[30rem] h-[30rem]">
        <img
          class="size-full"
          src="@/assets/mainPage_image.jpg"
          alt=""
          srcset=""
        />
      </div>
    </div>
  </div>
</template>

<style scoped></style>
