...
CREATE TABLE `guguns` (
  ...
  PRIMARY KEY (`no`),
  UNIQUE KEY `idx_gugun_sido_code_gugun_code` (`sido_code`,`gugun_code`),
  KEY `guguns_sido_to_sidos_code_fk_idx` (`sido_code`),
  CONSTRAINT `guguns_sido_to_sidos_code_fk` FOREIGN KEY (`sido_code`) REFERENCES `sidos` (`sido_code`) ON DELETE SET NULL ON UPDATE CASCADE
